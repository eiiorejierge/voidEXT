...
function adminEndpointAllowed(role, endpoint) {
  ...
  const adminOnly = new Set(['pool', 'link-add', 'link-remove', 'link-restore', 'link-delete', 'unblock', 'block', 'clear-reports', 'grant-tokens', 'operations', 'maintenance', 'usage-reset-all', 'reward-all', 'announce', 'announcements', 'releases', 'release', 'release-delete', 'delete-user']);
...
async function handle({ method, path, body, authHeader, ip }) {
  ...
  if (p === '/api/maintenance' && method === 'GET') {
    return { status: 200, json: { maintenance: publicMaintenance(await maintenanceInfo(true)) } };
  }
