...
async function handle({ method, path, body, authHeader, ip }) {
  ...
  // The gateway page. It embeds the target site in a full-viewport iframe whose
  // source is the SAME-ORIGIN loader below, so the browser address bar stays on
  // /go/<token> and the real destination never appears in the page HTML or the
  // URL bar. Served with a relaxed CSP so the frame is allowed to load.
  ...
  // The frame loader the gateway iframe points at. It redirects *inside the
  // iframe* to the real destination, so the target loads and stays usable while
  // the top-level address bar is untouched. Rate limited per IP so the pool
  // cannot be scraped in bulk. The destination is never returned to page JS.
  if (method === 'GET' && /^\/api\/frame\/[A-Za-z0-9_-]{6,64}$/.test(p)) {
    const token = p.slice('/api/frame/'.length);
    if (await cloak.rateLimited(ip)) {
      return { status: 429, contentType: 'text/html; charset=utf-8', body: cloak.notFoundPage() };
    }
    const dest = await cloak.resolveToken(token);
