...
// 00:00 UTC. The public UI deliberately exposes percentage, not the raw limit.
...
function accountInfo(user, now) {
  const period = usagePeriod(now);
  const used = weeklyUsed(user, now);
  const bonus = (user && user.bonus) || 0;
  return {
    created: (user && user.created) || null,
    used,
    bonus,
    remaining: Math.max(0, LINKS_PER_WEEK - used) + bonus,
    limit: LINKS_PER_WEEK,
    usagePercent: Math.max(0, Math.min(100, Math.round((used / LINKS_PER_WEEK) * 100))),
...
async function adminIdentity(authHeader) {
  const supplied = bearer(authHeader);
  if (ADMIN_KEY && supplied === ADMIN_KEY) return { role: 'owner', username: 'Owner', key: null, user: null };
...
function adminCapabilities(role) {
  if (role === 'owner') return ['overview', 'links', 'reports', 'support', 'accounts', 'accountsWrite', 'roles', 'messages', 'announcements', 'releases'];
  if (role === 'admin') return ['overview', 'links', 'reports', 'support', 'accounts', 'accountsWrite', 'messages', 'announcements', 'releases'];
  if (role === 'support') return ['overview', 'reports', 'support', 'accounts', 'messages'];
  return [];
}
function adminEndpointAllowed(role, endpoint) {
  if (role === 'owner') return true;
  const adminOnly = new Set(['pool', 'link-add', 'link-remove', 'link-restore', 'link-delete', 'unblock', 'block', 'clear-reports', 'grant-tokens', 'announce', 'announcements', 'releases', 'release', 'release-delete', 'delete-user']);
...
async function handle({ method, path, body, authHeader, ip }) {
  ...
  if (p === '/api/signup' && method === 'POST') {
    ...
    const newUser = { username, salt, hash, created: Date.now(), role: 'member', usage: {}, settings, links: [] };
  ...
  if (p === '/api/login' && method === 'POST') {
    ...
    const token = newToken();
    await store.set(`token:${token}`, key);
    return { status: 200, json: { token, username: user.username, settings: settingsOf(user), links: user.links || [], pinned: user.pinned || [], account: publicAccountInfo(user), notifications: user.notifications || [], unread: unread(user), messagesUnread: await totalUnreadMessages(key) } };
  }
  ...
  if (p === '/api/me' && method === 'GET') {
    ...
    return { status: 200, json: { username: auth.user.username, settings: settingsOf(auth.user), links: auth.user.links || [], pinned: auth.user.pinned || [], account: publicAccountInfo(auth.user), notifications: auth.user.notifications || [], unread: unread(auth.user), messagesUnread: await totalUnreadMessages(auth.key) } };
  }
  ...
  if (p === '/api/links' && method === 'GET') {
    ...
    if (RATE_LIMIT_ENABLED && totalRemaining <= 0)
      return {
        status: 429,
        json: { error: 'Weekly usage is at 100%. It resets Saturday.', links: user.links, added: null, usagePercent: 100, resetAt: period.resetAt },
      };
    ...
    if (!added) {
      ...
      return {
        status: 200,
        json: { links: user.links, added: null, poolSize: pool.length, note: 'No new links available right now.', rateLimit: RATE_LIMIT_ENABLED, usagePercent: info.usagePercent, resetAt: info.resetAt },
      };
    }
  ...
  // ---- admin: owner key or a signed-in staff account ------------------------
  if (p.startsWith('/api/admin/')) {
    ...
    if (p === '/api/admin/stats' && method === 'GET') {
      ...
      const roleCounts = users.reduce((out, user) => { out[roleOf(user)]++; return out; }, { member: 0, support: 0, admin: 0 });
    ...
    if (p === '/api/admin/users' && method === 'GET') {
      ...
      for (const key of keys) {
        ...
        users.push({
          ...
          bonus: u.bonus || 0,
          usagePercent: accountInfo(u).usagePercent,
          resetAt: accountInfo(u).resetAt,
          role: roleOf(u),
    ...
    // Only the owner key can promote or demote staff accounts.
