...
async function handle({ method, path, body, authHeader }) {
  ...
  if (p === '/api/signup' && method === 'POST') {
    ...
    await putUser(key, { username, salt, hash, created: Date.now(), usage: {} });
    const token = newToken();
    await store.set(`token:${token}`, key);
    return { status: 200, json: { token, username } };
  }
  ...
  if (p === '/api/me' && method === 'GET') {
    ...
    return { status: 200, json: { username: auth.user.username } };
  }
  ...
  if (p === '/api/links' && method === 'GET') {
    ...
    const { batch, poolSize } = await nextBatch(LINKS_PER_BATCH);
