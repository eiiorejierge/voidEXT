...
function sanitizeSettings(input) {
  ...
  if (input && typeof input === 'object') {
    ...
    if (!isNaN(n)) s.batchSize = Math.max(1, Math.min(LINKS_PER_DAY, n));
  }
  return s;
}
...
async function handle({ method, path, body, authHeader, ip }) {
  ...
  if (p === '/api/signup' && method === 'POST') {
    ...
    return { status: 200, json: { token, username, settings, links: [] } };
  }
  ...
  if (p === '/api/login' && method === 'POST') {
    ...
    const token = newToken();
    await store.set(`token:${token}`, key);
    return { status: 200, json: { token, username: user.username, settings: settingsOf(user), links: user.links || [] } };
  }
  ...
  if (p === '/api/me' && method === 'GET') {
    ...
    return { status: 200, json: { username: auth.user.username, settings: settingsOf(auth.user), links: auth.user.links || [] } };
  }
  ...
  if (p.startsWith('/api/admin/')) {
    ...
    if (p === '/api/admin/blocked' && method === 'GET') {
      ...
      const reports = await readReports();
      const users = await store.smembers('users');
      return { status: 200, json: { blocked, reports, totalLinks: LINKS.length, activeLinks: LINKS.length - blocked.length, accounts: users.length } };
    }
