...
async function handle({ method, path, body, authHeader, ip }) {
  ...
  if (p.startsWith('/api/admin/')) {
    ...
    if (p === '/api/admin/stats' && method === 'GET') {
      const [blocked, removed, custom, links, pool, userKeys, reports, bugs] = await Promise.all([
        store.smembers('blocked'),
        store.smembers('removedlinks'),
    ...
    const at = nextBugTime(bug);
      ...
      bug.messages.push({
        ...
        text: tokens > 0
          ? `Marked fixed. A reward of ${tokens} link token${tokens === 1 ? '' : 's'} was added to your account.`
