...
const BUILT_IN_RELEASES = [{
  ...
  text: 'Generated links are now cloaked behind an opaque, per-link gateway. Instead of a raw destination, you get and share a short Scale XT link that routes every visitor through the service before redirecting. The real destination is never shown, so links can no longer be copied out of the app or used to bypass the service, and blocking a link instantly disables every gateway link that points at it.',
...
async function handle({ method, path, body, authHeader, ip }) {
  ...
  // The branded pass-through page. The destination URL is never printed here;
  // the page fetches it from /api/go/<token> and only then redirects, so the
  // real link is never exposed by the gateway HTML itself.
  if (method === 'GET' && /^\/go\/[A-Za-z0-9_-]{6,64}$/.test(p)) {
    const token = p.slice('/go/'.length);
    const dest = await cloak.resolveToken(token);
    return {
      status: dest ? 200 : 404,
      contentType: 'text/html; charset=utf-8',
      body: dest ? cloak.gatewayPage(token) : cloak.notFoundPage(),
    };
  }

  // The resolver the gateway page calls. Returns the destination as JSON only —
  // rate limited per IP so the pool can't be scraped in bulk through it.
  if (method === 'GET' && /^\/api\/go\/[A-Za-z0-9_-]{6,64}$/.test(p)) {
    const token = p.slice('/api/go/'.length);
    if (await cloak.rateLimited(ip)) {
      return { status: 429, json: { error: 'Too many requests. Please slow down.' } };
    }
    const dest = await cloak.resolveToken(token);
    if (!dest) return { status: 404, json: { error: 'Link not found.' } };
    await incrementMetric('redirects');
    return { status: 200, json: { url: dest } };
  }
  ...
  if (p === '/api/login' && method === 'POST') {
    ...
    return { status: 200, json: { token, username: user.username, settings: settingsOf(user), links: await cloak.cloakList(user.links || []), pinned: await cloak.cloakList(user.pinned || []), account: publicAccountInfo(user), notifications: user.notifications || [], unread: unread(user), messagesUnread: await totalUnreadMessages(key) } };
  }
  ...
  if (p === '/api/me' && method === 'GET') {
    ...
    return { status: 200, json: { username: auth.user.username, settings: settingsOf(auth.user), links: await cloak.cloakList(auth.user.links || []), pinned: await cloak.cloakList(auth.user.pinned || []), account: publicAccountInfo(auth.user), notifications: auth.user.notifications || [], unread: unread(auth.user), messagesUnread: await totalUnreadMessages(auth.key) } };
  }
  ...
  if (p === '/api/links' && method === 'GET') {
    ...
    if (!added) {
      ...
      return {
        status: 200,
        json: Object.assign({ links: await cloak.cloakList(user.links), added: null, poolSize: pool.length, note: 'No new links available right now.', rateLimit: RATE_LIMIT_ENABLED }, info),
      };
    }
    ...
    return {
      status: 200,
      json: {
        links: await cloak.cloakList(user.links),
        added: await cloak.cloakUrl(added),
        ...
        resetAt: info.resetAt,
        pinned: await cloak.cloakList(user.pinned || []),
      },
    };
  }
  ...
  if (p === '/api/links/delete' && method === 'POST') {
    ...
    await putUser(key, user);
    return { status: 200, json: { ok: true, links: await cloak.cloakList(user.links), pinned: await cloak.cloakList(user.pinned) } };
  }
