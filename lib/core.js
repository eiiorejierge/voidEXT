...
const BUILT_IN_RELEASES = [{
  id: 'scale-xt-2.9.10',
  version: '2.9.10',
  title: 'Fresh links on usage reset',
  text: 'Weekly usage resets now clear the previous saved and pinned links at the same time, giving every account a clean set of five fresh base links. Old private-link URLs expire when they leave the saved set, and staff-triggered usage resets follow the same rule.',
  at: Date.UTC(2026, 7, 21, 2, 0, 0),
  source: 'built-in',
}, {
  id: 'scale-xt-2.9.9',
  version: '2.9.9',
  title: 'Delete confirmation',
  text: 'Deleting a saved link now opens a simple confirmation with Cancel and Delete. The link stays saved until deletion is confirmed, and the old Undo step has been removed.',
  at: Date.UTC(2026, 7, 21, 1, 0, 0),
  source: 'built-in',
}, {
  id: 'scale-xt-2.9.8',
  version: '2.9.8',
  title: 'Monochrome private-link authentication',
  text: 'The private-link authentication checker now matches the Scale XT bookmark with a black-and-white system theme, grid canvas, crisp status panel, monochrome loader, and matching owner-login instructions. Link ownership checks and destination privacy are unchanged.',
  at: Date.UTC(2026, 7, 21, 0, 0, 0),
  source: 'built-in',
}, {
  id: 'scale-xt-2.9.7',
  version: '2.9.7',
  title: 'Full-screen required updates',
  text: 'When a saved Scale XT bookmark is out of date, the update now takes over the entire bookmark screen instead of appearing as a small corner alert. The required screen shows the installed and latest versions, replacement steps, and the copy button, and stays open until the bookmark is updated.',
  at: Date.UTC(2026, 7, 20, 23, 0, 0),
  source: 'built-in',
}, {
  id: 'scale-xt-2.9.6',
  version: '2.9.6',
  title: 'Bookmark sign-in instructions',
  text: 'Private link pages no longer send users to the website app page. When a login is missing or expired, the page now tells users to open the Scale XT bookmark, sign in inside the bookmark, then return and refresh the private link.',
  at: Date.UTC(2026, 7, 20, 22, 0, 0),
  source: 'built-in',
}, {
  id: 'scale-xt-2.9.5',
  version: '2.9.5',
  title: 'Gateway-only login guidance',
  text: 'The login-required notice has been removed from the bookmark dashboard. Sign-in guidance now appears only when opening a private nebulabkmlinks.shop link without the required session, keeping the normal bookmark experience clean.',
  at: Date.UTC(2026, 7, 20, 21, 0, 0),
  source: 'built-in',
}, {
  id: 'scale-xt-2.9.4',
  version: '2.9.4',
  title: 'Clear private-link sign-in guidance',
  text: 'The Links dashboard now clearly explains that private links require an active nebulabkmlinks.shop login in the same browser. If a session is missing or expired, the locked viewer provides a direct sign-in button and simple instructions for returning to the link.',
  at: Date.UTC(2026, 7, 20, 20, 0, 0),
  source: 'built-in',
}, {
  id: 'scale-xt-2.9.3',
  version: '2.9.3',
  title: 'Dedicated link domain',
  text: 'Scale XT now opens the app and generates every personal, owner-locked link on nebulabkmlinks.shop. The dedicated domain keeps the launcher, account session, API, and private link viewer together so your links remain protected and continue working through the same fast gateway.',
  at: Date.UTC(2026, 7, 20, 18, 0, 0),
  source: 'built-in',
}, {
  id: 'scale-xt-2.9.2',
  version: '2.9.2',
...
function syncLinkPeriod(user, now, force) {
  const period = usagePeriod(now);
  const current = typeof user.linkPeriod === 'string' ? user.linkPeriod : '';
  if (!current) {
    // Existing accounts keep their current links for the rest of this week;
    // the next Saturday boundary starts the new clean-link behavior.
    user.linkPeriod = period.key;
    return { changed: true, cleared: 0, period };
  }
  if (force || current !== period.key) {
    const cleared = Array.isArray(user.links) ? user.links.length : 0;
    user.links = [];
    user.pinned = [];
    user.linkPeriod = period.key;
    return { changed: true, cleared, period };
  }
  return { changed: false, cleared: 0, period };
}
...
async function userFromAuth(authHeader) {
  ...
  const linkSync = user ? syncLinkPeriod(user) : null;
  if (linkSync && linkSync.changed) await putUser(key, user);
  if (user && legacy) {
...
async function handle({ method, path, body, authHeader, ip }) {
  ...
  if (p === '/api/frame-ticket' && method === 'POST') {
    ...
    if (resolved.owner && !(auth.user.links || []).includes(resolved.url)) {
      return { status: 404, json: { error: 'This link has expired.' } };
    }
  ...
  if (p === '/api/login' && method === 'POST') {
    ...
    const linkSync = syncLinkPeriod(user);
    if (linkSync.changed) await putUser(key, user);
    ...
    if (p === '/api/admin/usage-reset-all' && method === 'POST') {
      ...
      for (let index = 0; index < keys.length; index += 20) {
        const results = await Promise.all(keys.slice(index, index + 20).map(async (key) => {
          const user = await getUser(key);
          if (!user) return { usage: 0, links: 0 };
          const hadUsage = weeklyUsed(user) > 0 || weeklyCreditsUsed(user) > 0;
          const linkReset = syncLinkPeriod(user, undefined, true);
          const hadLinks = linkReset.cleared > 0;
          user.usage = user.usage || {};
          user.usage[period.key] = 0;
          user.usage[creditUsageKey(period)] = 0;
          if (hadUsage || hadLinks) {
            user.notifications = user.notifications || [];
            user.notifications.push({
              id: crypto.randomBytes(5).toString('hex'),
              text: 'Your weekly usage and saved links were reset by Scale XT staff.',
              at: Date.now(),
              read: false,
              from: 'admin',
            });
          ...
          await putUser(key, user);
          return { usage: hadUsage ? 1 : 0, links: linkReset.cleared };
        }));
        reset += results.reduce((sum, value) => sum + value.usage, 0);
        linksCleared += results.reduce((sum, value) => sum + value.links, 0);
      }
      return { status: 200, json: { ok: true, accounts: keys.length, reset, linksCleared, resetAt: period.resetAt } };
    }
...
module.exports = { handle, LINKS, RATE_LIMIT_ENABLED, LINKS_PER_WEEK, usagePeriod, syncLinkPeriod };
