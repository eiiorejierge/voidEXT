...
// ---- Config -----------------------------------------------------------------
// Daily quota. Each account may generate LINKS_PER_DAY links per UTC day.
// Generated links append to the user's saved set until they delete or report them.
...
const ADMIN_KEY = process.env.ADMIN_KEY || (process.env.NODE_ENV === 'production' ? '' : '7238');
...
async function handle({ method, path, body, authHeader, ip }) {
  ...
  if (p === '/api/login' && method === 'POST') {
    ...
    const token = newToken();
    await store.set(`token:${token}`, key);
    return { status: 200, json: { token, username: user.username, settings: settingsOf(user), links: user.links || [], pinned: user.pinned || [], account: accountInfo(user), notifications: user.notifications || [], unread: unread(user), messagesUnread: await totalUnreadMessages(key) } };
  }
  ...
  if (p === '/api/me' && method === 'GET') {
    ...
    return { status: 200, json: { username: auth.user.username, settings: settingsOf(auth.user), links: auth.user.links || [], pinned: auth.user.pinned || [], account: accountInfo(auth.user), notifications: auth.user.notifications || [], unread: unread(auth.user), messagesUnread: await totalUnreadMessages(auth.key) } };
  }
  ...
  if (p === '/api/links' && method === 'GET') {
    ...
    if (RATE_LIMIT_ENABLED && totalRemaining <= 0)
      return {
        status: 429,
        json: { error: `Daily limit reached (${LINKS_PER_DAY}/day). Try again tomorrow.`, links: user.links, added: null, remaining: 0, limit: LINKS_PER_DAY, used: usedToday, bonus },
      };
    ...
    if (!added) {
      ...
      return {
        status: 200,
        json: { links: user.links, added: null, poolSize: pool.length, note: 'No new links available right now.', rateLimit: RATE_LIMIT_ENABLED, remaining: RATE_LIMIT_ENABLED ? info.remaining : null, limit: LINKS_PER_DAY, used: info.used, bonus: info.bonus },
      };
    }
    ...
    return {
      ...
      json: {
        ...
        used: RATE_LIMIT_ENABLED ? info.used : null,
        bonus: RATE_LIMIT_ENABLED ? info.bonus : null,
        pinned: user.pinned || [],
      },
    };
  }
  ...
  if (p === '/api/links/delete' && method === 'POST') {
    ...
    if (!url) return { status: 400, json: { error: 'No link specified.' } };
    const { key, user } = auth;
    user.links = (user.links || []).filter((x) => x !== url);
    user.pinned = (user.pinned || []).filter((x) => x !== url);
    await putUser(key, user);
    return { status: 200, json: { ok: true, links: user.links, pinned: user.pinned } };
  }
  ...
  if (p === '/api/bug' && method === 'POST') {
    ...
    console.log(`[VoidEXT] BUG by ${auth.user.username}: ${text.slice(0, 80)}`);
    return { status: 200, json: { ok: true } };
  }
  ...
  // gift a link token to another user — rate limited to 1 per recipient per hour
  if (p === '/api/send-token' && method === 'POST') {
    const auth = await userFromAuth(authHeader);
    if (!auth) return { status: 401, json: { error: 'Not logged in.' } };
    const to = typeof body.to === 'string' ? body.to.toLowerCase() : '';
    if (!to) return { status: 400, json: { error: 'Pick a recipient.' } };
    if (to === auth.key) return { status: 400, json: { error: "You can't send tokens to yourself." } };
    const recipient = await getUser(to);
    if (!recipient) return { status: 404, json: { error: 'User not found.' } };
    ...
    if (await store.get(rlKey)) {
      ...
      return { status: 429, json: { error: `You can only send 1 token to ${recipient.username} per hour. Try again in ~${Math.ceil(ttl / 60)} min.` } };
    }

    // sender must have a token to give (spend a refunded bonus first, else daily)
    const sender = auth.user;
    const day = todayKey();
    sender.usage = sender.usage || {};
    const usedToday = sender.usage[day] || 0;
    const bonus = sender.bonus || 0;
    const available = Math.max(0, LINKS_PER_DAY - usedToday) + bonus;
    if (available < 1) return { status: 400, json: { error: "You don't have any tokens to send." } };
    if (bonus > 0) sender.bonus = bonus - 1;
    else sender.usage[day] = usedToday + 1;
    ...
    recipient.notifications.push({ id: crypto.randomBytes(5).toString('hex'), text: `${auth.user.username} sent you 1 link token.`, at: Date.now(), read: false, from: auth.user.username });
    ...
    return { status: 200, json: { ok: true, sentTo: recipient.username, account: accountInfo(sender) } };
  }
  ...
  if (p.startsWith('/api/admin/')) {
    ...
    if (!ADMIN_KEY) return { status: 503, json: { error: 'Admin access is not configured.' } };
    ...
    // announcement -> push the same notification into EVERY account's inbox
    if (p === '/api/admin/announce' && method === 'POST') {
      ...
      for (const k of keys) {
        ...
        if (!u) continue;
        u.notifications = u.notifications || [];
        u.notifications.push({ id: crypto.randomBytes(5).toString('hex'), text, at, read: false, from: 'announcement' });
        if (u.notifications.length > 50) u.notifications = u.notifications.slice(-50);
        await putUser(k, u);
        sent++;
      }
      // keep a log of announcements for the admin panel history
      await store.set(`announce:${aid}`, JSON.stringify({ id: aid, text, at, recipients: sent }));
      await store.rpush('announceids', aid);
      console.log(`[VoidEXT] ANNOUNCE to ${sent} account(s): ${text.slice(0, 80)}`);
      return { status: 200, json: { ok: true, recipients: sent } };
    }
    ...
    if (p === '/api/admin/announcements' && method === 'GET') {
      ...
      return { status: 200, json: { announcements: items } };
    }
