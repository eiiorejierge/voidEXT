  ...
  switch (op) {
    ...
    case 'LTRIM':
      return pgCollection(key, 'list', function (items) {
        ...
        const kept = items.slice(start, end + 1);
        items.length = 0;
        kept.forEach(function (item) { items.push(item); });
        return 'OK';
      });
    case 'LRANGE': {
      const row = await pgRow(key);
...
function memCmd(args) {
  ...
  switch (op) {
    ...
    case 'LTRIM': {
      ...
      mem.lists.set(key, list.slice(start, end + 1));
      return 'OK';
    }
    case 'LRANGE': {
      const list = mem.lists.get(key) || [];
...
module.exports = {
  ...
  ltrim: function (key, start, end) { return cmd(['LTRIM', key, start, end]); },
