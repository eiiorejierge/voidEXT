...
function warnFallback() {
  ...
  warned = true;
  console.warn(
    '[Scale XT] No KV/Upstash store configured (KV_REST_API_URL / KV_REST_API_TOKEN missing). ' +
...
function memCmd(args) {
  ...
  switch (String(op).toUpperCase()) {
    ...
    case 'SET':
      ...
      return 'OK';
    case 'DEL': {
      const existed = mem.kv.has(rest[0]) || mem.sets.has(rest[0]) || mem.lists.has(rest[0]);
      mem.kv.delete(rest[0]);
      mem.sets.delete(rest[0]);
      mem.lists.delete(rest[0]);
      mem.exp.delete(rest[0]);
      return existed ? 1 : 0;
    }
    case 'INCRBY': {
    ...
    case 'SCAN': {
      // args: SCAN <cursor> MATCH <pattern> COUNT <n>
      const mi = rest.indexOf('MATCH');
      const pattern = mi >= 0 ? rest[mi + 1] : '*';
      const re = new RegExp('^' + pattern.replace(/[.+^${}()|[\]\\]/g, '\\$&').replace(/\*/g, '.*') + '$');
      const keys = [...mem.kv.keys()].filter((k) => re.test(k));
      return ['0', keys];
    }
    case 'SREM': {
...
module.exports = {
  ...
  scan: async (pattern) => {
    ...
    do {
      ...
      if (Array.isArray(batch)) keys = keys.concat(batch);
    } while (cursor && cursor !== '0' && ++guard < 100);
    return keys;
  },
